# This workflow derives a simplified drainage network from GloFAS upstream-area data.
...
def load_and_prepare_upstream_area(
    ...
    """
    Load GloFAS upstream area, clean invalid values, convert from m² to km²,
    and subset to the target domain.
    """
...
def build_saber_drain_table(graphs):
    ...
    - SABER-ready fields
    - extra topology fields useful for later GLoFAS extraction/correction
    """
    rows = []
...
def save_drain_outputs(
    ...
    - full Parquet for later GLoFAS matching/correction
    """
    saber_cols = [
        "model_id",
        "downstream_model_id",
...
def build_glofas_drainage_network(cfg: dict | None = None) -> None:
