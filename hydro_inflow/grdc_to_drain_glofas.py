...
def filter_saber_ready_stations(
    ...
    logger.debug("Stations before Qobs coverage filter: %s", len(grdc_df))
...
def load_drain_table(path: Path) -> pd.DataFrame:
    ...
    logger.debug("Drain nodes loaded: %d", len(df))

    return df


# ======================================================
# MATCH STATIONS DIRECTLY TO DRAIN
# ======================================================
...
def build_match_row(
    ...
    grdc_lon = float(station["Longitude"])
    grdc_lat = float(station["Latitude"])
    target_area = float(station["Area_km2"])

    drain_lon = float(best["x"])
    drain_lat = float(best["y"])
    ...
    return {
        ...
        "match_type": "glofas_drain_nearest_then_radius_area_ratio",
        ...
        "grdc_lon": grdc_lon,
        "grdc_lat": grdc_lat,
        "glofas_lon": drain_lon,
        "glofas_lat": drain_lat,
        "drain_lon": drain_lon,
        "drain_lat": drain_lat,
...
def match_stations_to_drain(
    ...
    if drain.empty:
        raise ValueError("GLoFAS drain table is empty after cleaning.")
            ...
            no_match_rows.append(
                {
                    ...
                    "station_lat": lat,
                    "station_lon": lon,
                    "reason": f"no GLoFAS drain node within {max_distance_km:.0f} km bbox",
                }
            )
            continue
            ...
            no_match_rows.append(
                {
                    ...
                    "station_lon": lon,
                    "reason": (
                        f"GLoFAS drain nodes found within {max_distance_km:.0f} km, "
    ...
    logger.info("Station to GLoFAS drain matching completed.")
    logger.info("Stations mapped: %s", len(station_model_map))
    logger.info("Stations without GLoFAS drain match: %s", len(no_match_df))

    if not no_match_df.empty:
        logger.debug(
            "Stations without GLoFAS drain match preview:\n%s",
...
def log_drain_match_diagnostics(station_model_map: pd.DataFrame) -> None:
    ...
    logger.debug("GLoFAS drain match distance diagnostics")
    ...
    logger.debug(
        "Worst GLoFAS drain matches by distance:\n%s",
    ...
    logger.debug(
        "Worst GLoFAS drain matches by area_rel_diff:\n%s",
...
# ======================================================
# EXTRACT GLOFAS TIMESERIES
# ======================================================

def extract_glofas_timeseries_by_points(
    stations: pd.DataFrame,
    data_folder: Path,
    years: list[int],
    lat_col: str,
    ...
    logger.debug("GLoFAS station series to extract: %s", len(stations))
    ...
    if first_existing_file is None:
        raise FileNotFoundError(
            f"No GLoFAS files found in {data_folder} using template '{file_template}'"
        )
    ...
    if debug:
        logger.debug("GLoFAS extraction grid indices computed.")
    ...
    for year in years:
        ...
        if not filepath.exists():
            logger.warning(f"Missing GLoFAS file for year {year}: {filepath}")
            continue

        logger.debug(f"Processing GLoFAS {year}")
    ...
    logger.debug("GLoFAS Qsim extracted: %s", sum(v is not None for v in series_by_id.values()))
...
def write_station_qsim_from_dict(
    glofas_series_by_station: dict[str, pd.Series | None],
    ...
    for gauge_id, ts in glofas_series_by_station.items():
...
def save_overview_plot(
    ...
    if df_drain_roi.empty:
        logger.warning("No GLoFAS drain nodes available for overview plot.")
        return
    ...
    fig.add_trace(
        go.Scattergl(
            ...
            name="Matched GLoFAS drain nodes",
            text=(
                "Gauge: "
                + df_st["gauge_id"].astype(str)
                + "<br>Model ID: "
    ...
    for _, row in df_st.iterrows():
        link_x.extend([row["grdc_lon"], row["glofas_lon"], None])
        link_y.extend([row["grdc_lat"], row["glofas_lat"], None])
    ...
    fig.update_layout(
        title="GRDC stations, matched GLoFAS drain nodes and hydropower plants",
        width=1300,
        height=850,
...
def run_grdc_to_drain_glofas(cfg: dict | None = None) -> None:
    ...
    glofas_data_folder = cfg["hydro_data_dir"]
    ...
    plot_output_path = diagnostic_plot_dir / "stations_glofas_overview.html"
    ...
    logger.debug(
        "Final GRDC stations before GLoFAS drain mapping: %s",
        len(df_final),
    )
    ...
    glofas_series_by_station = extract_glofas_timeseries_by_points(
        stations=station_model_map,
        data_folder=glofas_data_folder,
        years=years,
        lat_col="glofas_lat",
        lon_col="glofas_lon",
        id_col="gauge_id",
    ...
    station_model_map, written_sim, skipped_sim = write_station_qsim_from_dict(
        glofas_series_by_station=glofas_series_by_station,
    ...
    logger.debug(
        "  Qsim extracted: %s",
        sum(v is not None for v in glofas_series_by_station.values()),
    )
    logger.debug("  Qobs extracted: %s", len(grdc_series_by_station))

    ds_grdc.close()

    logger.info("GRDC-to-GloFAS drain workflow completed.")
    logger.debug("Total time: %.1f s", time.time() - t0)


def main() -> None:
    setup_logging()
    run_grdc_to_drain_glofas()
